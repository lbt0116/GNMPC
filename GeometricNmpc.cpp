...
// 添加非线性动力学预测函数
void GeometricNmpc::predictStateTrajectory(const Matrix<double, Eigen::Dynamic, 1>& f)
{
    predicted_states.clear();
    Matrix<double, 13, 1> current_state = DeltaX;  // 初始误差状态

    for (int k = 0; k < Horizon; ++k)
    {
        // 提取当前步的控制输入
        Matrix<double, 3, 1> f_k = f.segment<3>(k * 3 * stanceLegNum);

        // 应用非线性动力学（示例：欧拉积分）
        Matrix<double, 13, 1> next_state;
        Matrix<double, 13, 13> A = getA(current_state);       // 需要实现基于当前状态的A矩阵
        Matrix<double, 13, Dynamic> B = getB(current_state);  // 需要实现基于当前状态的B矩阵

        next_state = current_state + dt_MPC * (A * current_state + B * f_k);
...
// 新增时变QP构建函数
void GeometricNmpc::buildTimeVaryingQP(const std::vector<Matrix<double, Kdim, Kdim>>& A_list,
                                       const std::vector<Matrix<double, Kdim, Dynamic>>& B_list,
                                       const Matrix<int, 4, 1>& Leg_Contact_State)
{
    // 重置QP矩阵
    A_qp.setZero();
    ...
    for (int i = 0; i < Horizon; ++i)
    {
        A_qp.block<Kdim, Kdim>(i * Kdim, i * Kdim) = A_list[i];
        B_qp.block(i * Kdim, i * 3 * stanceLegNum, Kdim, 3 * stanceLegNum) = B_list[i];
    }
...
void GeometricNmpc::run()
{
    try
    {
        ...
        // SQP参数
        const int max_sqp_iter = 3;
        const double tolerance = 1e-4;
        Matrix<double, Dynamic, 1> f_prev = f_qp;

        for (int sqp_iter = 0; sqp_iter < max_sqp_iter; ++sqp_iter)
        {
            // 1. 预测状态轨迹
            predictStateTrajectory(f_qp);

            // 2. 构建时变线性模型
            std::vector<Matrix<double, Kdim, Kdim>> A_list;
            std::vector<Matrix<double, Kdim, Dynamic>> B_list;

            for (int k = 0; k < Horizon; ++k)
            {
                auto& state_k = predicted_states[k];

                // 获取当前步的雅可比矩阵
                Matrix<double, Kdim, Kdim> A = getA(state_k);
                Matrix<double, Kdim, Dynamic> B = getB(state_k);

                // 离散化
                discreteAB(A, B);
                A_list.push_back(A_dt);
                B_list.push_back(B_dt);
            }

            // 3. 构建QP问题（时变模型）
            buildTimeVaryingQP(A_list, B_list, Leg_Contact_State);

            // 4. 求解QP
            initMPC_proxsuite(H_qp, g_qp, C_qp, d_qp, Leg_Contact_State);

            // 5. 检查收敛
            double delta_f = (f_qp - f_prev).norm();
            if (delta_f < tolerance) break;
            f_prev = f_qp;
        }
